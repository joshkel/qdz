...
function _M:init(title, x, y, filter, action, takeall, actor)
    ...
    self.action = action
    self.actor = actor
    Dialog.init(self, title or "Pick Up Items", math.max(800, game.w * 0.8), math.max(600, game.h * 0.8))

    -- Why 40?  I don't know, but it makes the default UI line up.
    local takeall = Button.new{text=takeall or "(*) Take all", width=math.floor(self.iw / 2 - 40), fct=function() self:takeAll() end}

    self.c_desc = TextzoneList.new{width=math.floor(self.iw / 2 - 10), height=self.ih - takeall.h, no_color_bleed=true}

    self:generateList()

    -- Create the list.  First, try letting it auto-size; if that's too big
    -- for the window, recreate it limited to window size.
    local list_max_height = self.ih - 20 - takeall.h
    local function make_list_columns(height)
        self.c_list = ListColumns.new{width=math.floor(self.iw / 2 - 10), height=height, scrollbar=true, columns={
        ...
        }, nb_items=#self.list, list=self.list, fct=function(item) self:use(item) end, select=function(item, sel) self:select(item) end}
    end
    make_list_columns()
    if self.c_list.h > list_max_height then
        make_list_columns(list_max_height)
    end

    self:loadUI{
        {left=0, top=0, ui=self.c_list},
        {right=0, top=0, ui=self.c_desc},
        {left=0, top=self.c_list.h + 20, ui=takeall},
        {hcenter=0, top=5, ui=Separator.new{dir="horizontal", size=self.ih - takeall.h - 10}},
    }
...
function _M:generateList()
    ...
    if self.c_list then self.c_list:setList(self.list) end
end
